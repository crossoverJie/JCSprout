...
由于在 MySQL 的 `innoDB` 引擎中采用的是聚集索引，在主键是尽量的保证写入是有序并递增的。

通常有以下几种方案：

## 基于数据库
可以利用 MySQL 中的自增属性 `auto_increment` 来生成全局唯一ID，也能保证趋势递增。
...
- 扩容增加性能困难，之前已经定义好了 A B 库递增的步数，新加的数据库不好加入进来，水平扩展困难。
...
可以基于 `Twitter` 的 `Snowflake` 算法来实现。